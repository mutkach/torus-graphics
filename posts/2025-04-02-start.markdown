...
Let's take for example `Llama-3.2-11B-Visual-Instruct`.
...
What we need to do first is find `forward()` in the `MLLaMAModel class` (located in `models/mllama/model.py`) and register the output just after the transformer block - this will get 39 debug outputs -  one for each layer - which we would be able inspect at runtime later:
...
![image00](../images/image1.png){.small}
...
![after crossatt](../images/image2.png){.small}
...
![image3](../images/image3.png){.small}
...
![image_last](../images/image4.png)

# Something is weird
Let's make a plot that shows the delta between layers `N` and `N-1`. And we will see something peculiar. Exactly between layers 3 and 2, where we can observe that complete masking happens. Our control tokens are completely **masked out** for all blocks in cross-attention in `TRT-LLM` implementation. 
Here's an (possibly not too confusing) image. 




![masked tokens](../images/image5.png)
...
1) cross-attention
2) incorrect masking
Is it enough to cause incorrect behavior downstream all the way to the incorrect answer? Possibly no, but it's definitely worth further investigation. And that's the only

# Conclusion
What did we learn today: 
* How to do some TensorRT-LLM instrumentation

* Patched some Torch models littering them all with our probes 
