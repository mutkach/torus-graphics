...
# Something is weird
Let's make a plot that shows the delta between layers `N` and `N-1`. And we will see something peculiar. Exactly between layers 3 and 2, where we can observer that complete masking happens. Our control tokens are completely **masked out** for all blocks in cross-attention in `TRT-LLM` implementation. 
Here's an (possibly not too confusing) image. 




![masked tokens](../images/diffs4.png)
...
1) cross-attention
2) incorrect masking
Is it enough to cause incorrect behavior downstream all the way to the incorrect answer? Possibly no, but it's definitely worth further investigation. 
