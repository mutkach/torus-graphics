...
Is it enough of to cause incorrect behavior all the way downstream to the incorrect answer? Possibly no, but it's definitely worth further investigation. And that's the only
...
In the next episode we will check if Nvidia patched their implementation and also it would be nice to extract attention masks and make some further observations.
